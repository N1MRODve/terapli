  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(cargandoSesiones)) {
        ...
        _push(`<div class="text-cafe/50 text-center py-10" data-v-d953f9ea>No hay sesiones próximas.</div>`);
      } else {
        _push(`<div class="space-y-4" data-v-d953f9ea><!--[-->`);
        ssrRenderList(unref(proximasSesiones), (cita) => {
          _push(`<div class="flex items-center justify-between bg-terracota/5 rounded-xl p-4 hover:bg-terracota/10 transition" data-v-d953f9ea><div data-v-d953f9ea><p class="text-lg font-semibold text-cafe" data-v-d953f9ea>${ssrInterpolate(cita.hora_inicio)} — ${ssrInterpolate(cita.paciente_nombre)}</p><p class="text-sm text-cafe/70" data-v-d953f9ea>${ssrInterpolate(cita.modalidad === "online" ? "Online 💻" : "Presencial 🏥")}</p></div><button class="px-3 py-1 text-sm border border-terracota/30 rounded-lg text-terracota hover:bg-terracota hover:text-white transition" data-v-d953f9ea> Ver detalles </button></div>`);
        });
        _push(`<!--]--></div>`);
      }
      _push(`</section><section class="card" data-v-d953f9ea><header class="flex justify-between items-center mb-4" data-v-d953f9ea><h2 class="text-xl font-semibold text-cafe" data-v-d953f9ea>🧍 Pacientes Activos</h2>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
      ...
      if (unref(cargandoPacientes)) {
        ...
        ssrRenderList(unref(pacientesActivos), (paciente) => {
          _push(`<div class="flex items-center justify-between bg-white border border-cafe/5 rounded-xl p-4 hover:shadow-sm transition" data-v-d953f9ea><div class="flex-1" data-v-d953f9ea><p class="font-semibold text-cafe" data-v-d953f9ea>${ssrInterpolate(paciente.nombre_completo)}</p><p class="text-sm text-cafe/60" data-v-d953f9ea>Última sesión: ${ssrInterpolate(paciente.ultima_sesion || "Sin registro")}</p><div class="w-full bg-base-bg h-1 mt-2 rounded-full" data-v-d953f9ea><div class="h-1 rounded-full bg-green-500 transition-all" style="${ssrRenderStyle({ width: `${paciente.progreso_bono || 0}%` })}" data-v-d953f9ea></div></div></div>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex items-center justify-center gap-2 mb-1" data-v-d953f9ea${_scopeId}><span class="text-2xl group-hover:scale-110 transition-transform" data-v-d953f9ea${_scopeId}>💶</span><p class="text-2xl font-bold text-green-700" data-v-d953f9ea${_scopeId}>${ssrInterpolate(formatearPrecio(unref(totalConfirmado)))}€</p></div><p class="text-sm text-cafe/60" data-v-d953f9ea${_scopeId}>Pagos confirmados</p><p class="text-xs text-green-700 font-medium mt-1" data-v-d953f9ea${_scopeId}>${ssrInterpolate(unref(totalBonosPagados))} ${ssrInterpolate(unref(totalBonosPagados) === 1 ? "bono" : "bonos")}</p>`);
          } else {
            return [
              createVNode("div", { class: "flex items-center justify-center gap-2 mb-1" }, [
                createVNode("span", { class: "text-2xl group-hover:scale-110 transition-transform" }, "💶"),
      ...
      _push(`</div></section><section class="card" data-v-d953f9ea><header class="flex justify-between items-center mb-4" data-v-d953f9ea><h2 class="text-xl font-semibold text-cafe" data-v-d953f9ea>💬 Recordatorios</h2>`);
      if (!unref(cargandoRecordatorios)) {
        _push(`<button class="text-xs text-terracota hover:text-cafe transition" title="Actualizar recordatorios" data-v-d953f9ea> 🔄 </button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</header>`);
      if (unref(cargandoRecordatorios)) {
        ...
        ssrRenderList(unref(recordatorios), (msg, i) => {
          _push(`<li class="flex items-start gap-2 bg-terracota/5 rounded-lg px-3 py-2 hover:bg-terracota/10 transition" data-v-d953f9ea><span class="flex-shrink-0 mt-0.5" data-v-d953f9ea>🔔</span><span class="flex-1" data-v-d953f9ea>${ssrInterpolate(msg)}</span></li>`);
        });
        _push(`<!--]--></ul>`);
      } else {
        _push(`<div class="text-cafe/60 text-sm py-8 text-center" data-v-d953f9ea> No hay recordatorios pendientes 🎉 </div>`);
      }
      _push(`</section></div>`);
