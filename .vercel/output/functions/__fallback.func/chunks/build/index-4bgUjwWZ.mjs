  ...
  setup(__props) {
    ...
    const sesionesFiltradas = computed(() => {
      ...
      if (filtros.value.busqueda) {
        ...
        resultado = resultado.filter(
          (s) => {
            var _a, _b;
            return (_b = (_a = s.paciente) == null ? void 0 : _a.nombre_completo) == null ? void 0 : _b.toLowerCase().includes(busqueda);
          }
        );
      }
      ...
      sesionesFiltradas.value.forEach((sesion) => {
        ...
        switch (sesion.estado) {
          case "pendiente":
            ...
            if (sesion.esta_pagado || ((_a = sesion.bono) == null ? void 0 : _a.pagado)) {
          ...
          case "confirmada":
            ...
            if (sesion.esta_pagado || ((_b = sesion.bono) == null ? void 0 : _b.pagado)) {
              resultado.montoConfirmado += montoTerapeuta;
            } else {
              resultado.montoPendiente += montoTerapeuta;
            }
          ...
          case "completada":
            ...
            if (sesion.esta_pagado || ((_c = sesion.bono) == null ? void 0 : _c.pagado)) {
              resultado.montoConfirmado += montoTerapeuta;
            } else {
    ...
    const bonosPagados = computed(() => {
      return bonosPagadosDirectos.value.map((bono) => {
        ...
        return {
          ...
          tipo_bono: bono.tipo_bono || "Est\xE1ndar",
    ...
    const cargarSesiones = async () => {
      try {
        ...
        console.log("\u{1F464} Usuario verificado:", user.value.id);
        ...
        if (!terapeuta) {
          throw new Error("No se encontr\xF3 el terapeuta asociado a este usuario");
        }
        ...
        console.log("Sesiones cargadas:", (data == null ? void 0 : data.length) || 0, data);
        sesiones.value = (data || []).map((sesion) => {
          console.log("Sesi\xF3n individual:", {
            id: sesion.id,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-base-bg" }, _attrs))}><div class="mb-8"><h1 class="text-3xl font-serif font-bold text-cafe mb-2"> Gesti\xF3n de Sesiones </h1><p class="text-terracota"> Visualiza y gestiona todas tus sesiones con informaci\xF3n financiera </p></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8"><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center justify-between mb-4"><div class="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center">`);
      ...
      if (cargando.value) {
        ...
        if (sesionesFiltradas.value.length === 0) {
          ...
          _push(`</div><h3 class="text-lg font-semibold text-cafe mb-2"> No hay sesiones para mostrar </h3><p class="text-cafe/60 mb-6">${ssrInterpolate(filtros.value.busqueda || filtros.value.estado ? "Intenta ajustar los filtros de b\xFAsqueda" : "No tienes sesiones programadas a\xFAn")}</p></div>`);
        } else {
          ...
          ssrRenderList(sesionesFiltradas.value, (sesion) => {
            ...
            ])}"><span>${ssrInterpolate(sesion.modalidad === "online" ? "\u{1F4BB}" : "\u{1F3E2}")}</span> ${ssrInterpolate(sesion.modalidad === "online" ? "Online" : "Presencial")}</span></td><td class="px-6 py-4 whitespace-nowrap"><span class="${ssrRenderClass([
...
//# sourceMappingURL=index-4bgUjwWZ.mjs.map
