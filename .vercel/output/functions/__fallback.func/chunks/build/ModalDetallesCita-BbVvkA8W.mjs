...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const cargarCita = async () => {
      var _a, _b;
      console.log("\u{1F50D} cargarCita - citaId:", props.citaId);
      if (!props.citaId) {
      ...
      `).eq("id", props.citaId).single();
        console.log("\u{1F4CA} Resultado query cita:", { citaData, citaError });
        if (citaError) {
        ...
        console.log("\u{1F464} Paciente:", paciente.value);
        if (citaData.bono_id) {
          console.log("\u{1F39F}\uFE0F Cargando bono:", citaData.bono_id);
          const { data: bonoData, error: bonoError } = await supabase.from("bonos").select("*").eq("id", citaData.bono_id).single();
          console.log("\u{1F4CA} Resultado query bono:", { bonoData, bonoError });
          if (!bonoError) {
            bono.value = bonoData;
            ...
            if (!citasError && citasAnteriores) {
              ...
              if (indice !== -1) {
                ...
                console.log("\u{1F4CD} N\xFAmero de sesi\xF3n:", bono.value.numero_sesion);
              }
            }
          }
        }
        formEdicion.value = {
          ...
          hora_inicio: ((_a = citaData.hora_inicio) == null ? void 0 : _a.substring(0, 5)) || "",
          hora_fin: ((_b = citaData.hora_fin) == null ? void 0 : _b.substring(0, 5)) || "",
        ...
        console.log("\u{1F3C1} Carga finalizada. Estado:", { cita: cita.value, paciente: paciente.value, bono: bono.value });
      }
    };
    watch(() => props.citaId, (newId, oldId) => {
      console.log("\u{1F440} Watch citaId cambi\xF3:", { oldId, newId });
      if (newId) {
        cargarCita();
      } else {
        console.log("\u{1F9F9} Limpiando estado del modal");
        cita.value = null;
        paciente.value = null;
        bono.value = null;
      }
    ...
    const fechaFormateada = computed(() => {
      var _a;
      if (!((_a = cita.value) == null ? void 0 : _a.fecha_cita)) return "";
    ...
    const claseEstado = computed(() => {
      var _a;
      const estado = (_a = cita.value) == null ? void 0 : _a.estado;
      const clases = {
    ...
    const estadoLabel = computed(() => {
      ...
      return labels[(_a = cita.value) == null ? void 0 : _a.estado] || ((_b = cita.value) == null ? void 0 : _b.estado) || "";
    });
        ...
        if (__props.citaId) {
          ...
          if (cargando.value) {
            ...
            if (!modoEdicion.value) {
              ...
              if ((_c = paciente.value) == null ? void 0 : _c.telefono) {
              ...
              if ((_d = paciente.value) == null ? void 0 : _d.frecuencia) {
              ...
              _push2(`<p class="text-xs font-semibold text-gray-500 uppercase tracking-wider" data-v-66c26338>Horario</p></div><p class="font-bold text-gray-900" data-v-66c26338>${ssrInterpolate((_e = cita.value.hora_inicio) == null ? void 0 : _e.substring(0, 5))} - ${ssrInterpolate((_f = cita.value.hora_fin) == null ? void 0 : _f.substring(0, 5))}</p></div><div class="bg-white rounded-xl p-4 shadow-sm border border-gray-200" data-v-66c26338><div class="flex items-center gap-2 mb-2" data-v-66c26338>`);
              ...
              _push2(`<p class="text-xs font-semibold text-gray-500 uppercase tracking-wider" data-v-66c26338>Duraci\xF3n</p></div><p class="font-bold text-gray-900" data-v-66c26338>${ssrInterpolate(cita.value.duracion_minutos || 60)} minutos</p></div></div>`);
              if (bono.value) {
                ...
                _push2(`</div><div data-v-66c26338><h3 class="font-bold text-emerald-900 text-lg" data-v-66c26338>Bono Activo</h3><p class="text-xs text-emerald-700" data-v-66c26338>Informaci\xF3n del paquete de sesiones</p></div></div>`);
                if (bono.value.numero_sesion) {
                  _push2(`<div class="bg-white px-4 py-2 rounded-full border-2 border-emerald-400 shadow-sm" data-v-66c26338><span class="text-sm font-black text-emerald-700" data-v-66c26338>Sesi\xF3n #${ssrInterpolate(bono.value.numero_sesion)}</span></div>`);
                } else {
                  _push2(`<!---->`);
                }
...
//# sourceMappingURL=ModalDetallesCita-BbVvkA8W.mjs.map
