    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.modelValue) {
          ...
          _push2(`</div><div data-v-0b1ffb67><label class="block text-sm font-[&#39;Lato&#39;] font-semibold text-[#5D4A44] mb-2" data-v-0b1ffb67> Mensaje opcional </label><textarea rows="3" placeholder="Añade un mensaje personalizado para el paciente..." class="w-full px-4 py-3 bg-white border border-[#EAD5D3]/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D8AFA0] text-[#5D4A44] resize-none" data-v-0b1ffb67>${ssrInterpolate(mensaje.value)}</textarea><p class="text-xs text-[#5D4A44]/60 mt-1" data-v-0b1ffb67> Este mensaje se incluirá en la notificación que recibirá el paciente </p></div>`);
          if (error.value) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.modelValue) {
          ...
          _push2(`<!--]--></div></div></div><div data-v-bba9cf3f><label class="block text-sm font-[&#39;Lato&#39;] font-semibold text-[#5D4A44] mb-2" data-v-bba9cf3f> Etiquetas </label><input${ssrRenderAttr("value", tagsInput.value)} type="text" placeholder="respiración, mindfulness, ansiedad (separadas por comas)" class="w-full px-4 py-3 bg-white border border-[#EAD5D3]/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D8AFA0] text-[#5D4A44]" data-v-bba9cf3f><p class="text-xs text-[#5D4A44]/60 mt-1" data-v-bba9cf3f> Separa las etiquetas con comas </p>`);
          if (form.value.tags.length > 0) {
            _push2(`<div class="flex flex-wrap gap-2 mt-2" data-v-bba9cf3f><!--[-->`);
            ssrRenderList(form.value.tags, (tag, index) => {
              _push2(`<span class="px-3 py-1 bg-[#D8AFA0]/20 text-[#5D4A44] rounded-full text-sm flex items-center gap-2" data-v-bba9cf3f> #${ssrInterpolate(tag)} <button type="button" class="text-[#5D4A44]/50 hover:text-[#5D4A44]" data-v-bba9cf3f> × </button></span>`);
            });
            _push2(`<!--]--></div>`);
          } else {
            _push2(`<!---->`);
  ...
  setup(__props) {
    ...
    const recursosFiltrados = computed(() => {
      ...
      return recursos2.value.filter(
        (r) => r.titulo?.toLowerCase().includes(termino) || r.descripcion?.toLowerCase().includes(termino) || r.categoria?.toLowerCase().includes(termino) || r.tipo?.toLowerCase().includes(termino)
      );
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(cargando)) {
        ...
        ssrRenderList(unref(recursosFiltrados), (recurso) => {
          ...
          _push(`<div class="flex flex-col gap-2" data-v-ce5bceaf><div class="flex gap-2" data-v-ce5bceaf><a${ssrRenderAttr("href", recurso.url)} target="_blank" class="flex-1 text-center px-3 py-2 text-sm bg-[#F9F7F3] text-[#5D4A44] rounded-lg hover:bg-[#EAD5D3]/30 transition-colors" data-v-ce5bceaf> 👁️ Ver </a><button class="flex-1 px-3 py-2 text-sm bg-[#D8AFA0] text-white rounded-lg hover:bg-[#D8AFA0]/90 transition-colors" data-v-ce5bceaf> 📤 Compartir </button></div><div class="flex gap-2" data-v-ce5bceaf><button class="flex-1 px-3 py-2 text-sm bg-white border border-[#EAD5D3] text-[#5D4A44] rounded-lg hover:bg-[#F9F7F3] transition-colors" data-v-ce5bceaf> ✏️ Editar </button>`);
          if (!recurso.id?.startsWith("demo")) {
            _push(`<button class="flex-1 px-3 py-2 text-sm bg-white border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition-colors" data-v-ce5bceaf> 🗑️ Eliminar </button>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div></div></div>`);
