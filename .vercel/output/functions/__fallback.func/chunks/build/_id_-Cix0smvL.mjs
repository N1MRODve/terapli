...
import { g as useSupabaseClient, i as useRoute, a as useRouter } from './server.mjs';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const tabs = [
      { id: "info", label: "Informaci\xF3n", icon: "\u{1F464}" },
      { id: "citas", label: "Citas", icon: "\u{1F4C5}" },
      { id: "bonos", label: "Bonos", icon: "\u{1F39F}\uFE0F" },
      { id: "pagos", label: "Pagos", icon: "\u{1F4B3}" },
      { id: "sesiones", label: "Sesiones", icon: "\u{1F4DD}" }
    ];
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(cargando)) {
        _push(`<div class="text-center py-12"><div class="animate-spin w-12 h-12 border-4 border-terracota border-t-transparent rounded-full mx-auto mb-4"></div><p class="text-cafe/60">Cargando informaci\xF3n del paciente...</p></div>`);
      } else if (unref(error)) {
        _push(`<div class="text-center py-12"><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8"><span class="text-6xl mb-4 block">\u274C</span><h3 class="text-xl font-serif font-semibold text-cafe mb-2">No se pudo cargar la informaci\xF3n</h3><p class="text-cafe/60 mb-4">${ssrInterpolate(unref(error))}</p><button class="px-6 py-3 bg-terracota text-white rounded-lg hover:bg-terracota/90 transition-colors"> Volver a la lista </button></div></div>`);
      } else if (unref(pacienteData)) {
        _push(`<div class="space-y-6"><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6"><div class="flex items-start gap-4 flex-1"><div class="w-20 h-20 rounded-full bg-gradient-to-br from-terracota to-cafe flex items-center justify-center text-white font-bold text-2xl flex-shrink-0">${ssrInterpolate(unref(iniciales))}</div><div class="flex-1"><div class="flex items-center gap-3 mb-2"><h1 class="text-3xl font-serif font-bold text-cafe">${ssrInterpolate(unref(nombreCompleto))}</h1><button class="text-gray-400 hover:text-terracota transition-colors" title="Editar informaci\xF3n"><span class="text-xl">\u270F\uFE0F</span></button></div><div class="flex flex-wrap items-center gap-3 mb-3"><span class="${ssrRenderClass([unref(pacienteData).activo ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700", "px-3 py-1 text-sm font-medium rounded-full"])}">${ssrInterpolate(unref(pacienteData).activo ? "Activo" : "Inactivo")}</span><span class="text-cafe/60 text-sm flex items-center gap-1"><span>\u{1F4E7}</span><span>${ssrInterpolate(unref(pacienteData).email)}</span></span></div><div class="space-y-1 text-sm">`);
        if (unref(pacienteData).telefono) {
          _push(`<div class="text-cafe/70 flex items-center gap-2"><span>\u{1F4F1}</span><span>${ssrInterpolate(unref(pacienteData).telefono)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(pacienteData).area_de_acompanamiento) {
          _push(`<div class="text-cafe/70 flex items-center gap-2"><span>\u{1F3AF}</span><span><strong>\xC1rea:</strong> ${ssrInterpolate(unref(pacienteData).area_de_acompanamiento)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(pacienteData).frecuencia) {
          _push(`<div class="text-cafe/70 flex items-center gap-2"><span>\u{1F4C6}</span><span><strong>Frecuencia:</strong> ${ssrInterpolate(unref(pacienteData).frecuencia)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div></div><div class="flex flex-col gap-2">`);
        if (unref(pacienteData).telefono) {
          _push(`<button class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm flex items-center gap-2 justify-center"><span>\u{1F4AC}</span><span>WhatsApp</span></button>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<button class="px-4 py-2 bg-terracota text-white rounded-lg hover:bg-terracota/90 transition-colors text-sm flex items-center gap-2 justify-center"><span>\u{1F4C5}</span><span>Agendar Cita</span></button></div></div></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4"><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center gap-3"><span class="text-3xl">\u{1F4CA}</span><div><p class="text-sm text-gray-600">Total Citas</p><p class="text-2xl font-bold text-cafe">${ssrInterpolate(unref(totalCitas))}</p></div></div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center gap-3"><span class="text-3xl">\u2705</span><div><p class="text-sm text-gray-600">Completadas</p><p class="text-2xl font-bold text-green-600">${ssrInterpolate(unref(citasCompletadas))}</p></div></div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center gap-3"><span class="text-3xl">\u{1F4C5}</span><div><p class="text-sm text-gray-600">Pr\xF3xima Cita</p><p class="text-sm font-semibold text-cafe">${ssrInterpolate(unref(proximaCitaTexto))}</p></div></div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center gap-3"><span class="text-3xl">\u{1F39F}\uFE0F</span><div><p class="text-sm text-gray-600">Bonos Activos</p><p class="text-2xl font-bold text-terracota">${ssrInterpolate(unref(bonosActivos).length)}</p></div></div></div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100"><div class="flex border-b border-gray-200 overflow-x-auto"><!--[-->`);
        ...
        if (unref(tabActual) === "bonos") {
          ...
          if (unref(cargandoBonos)) {
            ...
            _push(`<div class="text-center py-12 text-gray-400"><span class="text-6xl block mb-4">\u{1F39F}\uFE0F</span><p class="mb-4">No hay bonos registrados</p></div>`);
          } else {
            ...
            ssrRenderList(unref(bonos), (bono) => {
              ...
              if (bono.fecha_expiracion) {
                _push(`<div class="mt-3 text-sm text-gray-600"><span>\u{1F4C5} Expira: ${ssrInterpolate(formatearFechaCorta(bono.fecha_expiracion))}</span></div>`);
              } else {
                _push(`<!---->`);
              }
        ...
        if (unref(tabActual) === "pagos") {
          ...
          if (unref(cargandoPagos)) {
            ...
            _push(`<div class="text-center py-12 text-gray-400"><span class="text-6xl block mb-4">\u{1F4B3}</span><p class="mb-4">No hay pagos registrados</p></div>`);
          } else {
            _push(`<div class="space-y-3"><!--[-->`);
            ssrRenderList(unref(pagos), (pago) => {
              _push(`<div class="p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors"><div class="flex items-start justify-between"><div class="flex-1"><div class="flex items-center gap-3 mb-2"><span class="text-lg">\u{1F4B3}</span><div><p class="font-semibold text-cafe">${ssrInterpolate(pago.concepto || "Pago de sesi\xF3n")}</p><p class="text-sm text-gray-600">${ssrInterpolate(formatearFecha(pago.fecha))}</p></div></div><div class="flex items-center gap-2 mt-2"><span class="${ssrRenderClass([getPagoEstadoClasses(pago.estado), "px-2 py-1 text-xs rounded-full"])}">${ssrInterpolate(getPagoEstadoLabel(pago.estado))}</span>`);
              if (pago.metodo_pago) {
        ...
        if (unref(tabActual) === "sesiones") {
          ...
          if (unref(cargandoSesiones)) {
            ...
            _push(`<div class="text-center py-12 text-gray-400"><span class="text-6xl block mb-4">\u{1F4DD}</span><p class="mb-4">No hay sesiones registradas</p></div>`);
          } else {
            _push(`<div class="space-y-4"><!--[-->`);
...
//# sourceMappingURL=_id_-Cix0smvL.mjs.map
