...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        _push(`<div class="flex items-center justify-center py-20" data-v-f7095ca9><div class="text-center" data-v-f7095ca9><div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-[#D8AFA0]" data-v-f7095ca9></div><p class="mt-4 text-[#5D4A44]/60" data-v-f7095ca9>Cargando sesión...</p></div></div>`);
      } else if (error.value) {
        _push(`<div class="bg-red-50 border border-red-200 rounded-xl p-6" data-v-f7095ca9><h3 class="text-red-800 font-[&#39;Lora&#39;] text-lg mb-2" data-v-f7095ca9>Error al cargar la sesión</h3><p class="text-red-600 text-sm" data-v-f7095ca9>${ssrInterpolate(error.value)}</p>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/terapeuta/sesiones",
        ...
        _push(`<div class="space-y-6" data-v-f7095ca9><div class="bg-white rounded-xl shadow-sm p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><div class="flex items-start justify-between mb-4" data-v-f7095ca9><div data-v-f7095ca9><h1 class="text-3xl font-[&#39;Lora&#39;] text-[#5D4A44] mb-2" data-v-f7095ca9> Sesión #${ssrInterpolate(sesion.value.id.slice(0, 8))}</h1><p class="text-[#5D4A44]/70" data-v-f7095ca9>${ssrInterpolate(formatearFecha(sesion.value.fecha))} a las ${ssrInterpolate(formatearHora(sesion.value.fecha))}</p></div><span class="${ssrRenderClass([getEstadoClase(sesion.value.estado), "px-4 py-2 rounded-full text-sm font-medium"])}" data-v-f7095ca9>${ssrInterpolate(getEstadoTexto(sesion.value.estado))}</span></div></div><div class="grid grid-cols-1 lg:grid-cols-3 gap-6" data-v-f7095ca9><div class="lg:col-span-2 space-y-6" data-v-f7095ca9><div class="bg-white rounded-xl shadow-sm p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><h2 class="font-[&#39;Lora&#39;] text-xl text-[#5D4A44] mb-4 flex items-center gap-2" data-v-f7095ca9><span class="text-2xl" data-v-f7095ca9>📋</span> Detalles de la Sesión </h2><div class="grid grid-cols-2 gap-4 text-[#5D4A44]" data-v-f7095ca9><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Paciente</p>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(sesion.value.pacientes?.nombre || "Sin nombre")} <span class="text-xs" data-v-f7095ca9${_scopeId}>→</span>`);
            } else {
              return [
                createTextVNode(toDisplayString(sesion.value.pacientes?.nombre || "Sin nombre") + " ", 1),
        ...
        _push(`</div><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Fecha</p><p class="font-medium" data-v-f7095ca9>${ssrInterpolate(formatearFecha(sesion.value.fecha))}</p></div><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Hora</p><p class="font-medium" data-v-f7095ca9>${ssrInterpolate(formatearHora(sesion.value.fecha))}</p></div><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Modalidad</p><span class="inline-flex items-center gap-1 px-3 py-1 bg-[#F9F7F3] rounded-full text-sm" data-v-f7095ca9><span data-v-f7095ca9>${ssrInterpolate(sesion.value.modalidad === "online" ? "💻" : "🏥")}</span><span data-v-f7095ca9>${ssrInterpolate(sesion.value.modalidad === "online" ? "Online" : "Presencial")}</span></span></div><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Duración</p><p class="font-medium" data-v-f7095ca9>${ssrInterpolate(sesion.value.duracion || 50)} minutos</p></div><div data-v-f7095ca9><p class="text-sm text-[#5D4A44]/60 mb-1" data-v-f7095ca9>Bono vinculado</p><p class="font-medium" data-v-f7095ca9>${ssrInterpolate(sesion.value.bono_id ? "Sí" : "Sesión individual")}</p></div></div></div><div class="bg-gradient-to-br from-[#D8AFA0]/10 to-[#EAD5D3]/20 rounded-xl shadow-sm p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><h3 class="text-lg font-[&#39;Lora&#39;] text-[#5D4A44] mb-4 flex items-center gap-2" data-v-f7095ca9><span class="text-2xl" data-v-f7095ca9>💰</span> Resumen Financiero </h3><div class="space-y-3" data-v-f7095ca9><div class="flex justify-between items-center" data-v-f7095ca9><span class="text-[#5D4A44]/70" data-v-f7095ca9>Precio total de la sesión</span><span class="text-xl font-bold text-[#5D4A44]" data-v-f7095ca9>${ssrInterpolate(formatearPrecio(sesion.value.precio_total || 50))}</span></div><div class="border-t border-[#EAD5D3]/40 pt-3 space-y-2" data-v-f7095ca9><div class="flex justify-between items-center" data-v-f7095ca9><span class="text-[#5D4A44]/70" data-v-f7095ca9>Terapeuta (70%)</span><span class="text-lg font-semibold text-green-700" data-v-f7095ca9>${ssrInterpolate(formatearPrecio((sesion.value.precio_total || 50) * 0.7))}</span></div><div class="flex justify-between items-center" data-v-f7095ca9><span class="text-[#5D4A44]/70" data-v-f7095ca9>Consulta (30%)</span><span class="text-lg font-medium text-[#5D4A44]/80" data-v-f7095ca9>${ssrInterpolate(formatearPrecio((sesion.value.precio_total || 50) * 0.3))}</span></div></div><div class="border-t border-[#EAD5D3]/40 pt-3" data-v-f7095ca9><div class="flex justify-between items-center" data-v-f7095ca9><span class="text-[#5D4A44]/70" data-v-f7095ca9>Estado del pago</span><span class="${ssrRenderClass([sesion.value.pagado ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700", "px-3 py-1 rounded-full text-sm font-medium"])}" data-v-f7095ca9>${ssrInterpolate(sesion.value.pagado ? "✓ Pagado" : "⏳ Pendiente")}</span></div></div></div></div><div class="bg-white rounded-xl shadow-sm p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><div class="flex items-center justify-between mb-4" data-v-f7095ca9><h3 class="text-lg font-[&#39;Lora&#39;] text-[#5D4A44] flex items-center gap-2" data-v-f7095ca9><span class="text-2xl" data-v-f7095ca9>📝</span> Notas Privadas del Terapeuta </h3>`);
        ...
        _push(`</div><textarea class="w-full h-48 border border-[#EAD5D3] rounded-lg p-4 bg-[#F9F7F3] font-[&#39;Lato&#39;] text-[#5D4A44] focus:ring-2 focus:ring-[#D8AFA0] focus:border-transparent outline-none resize-none" placeholder="Anota observaciones, temas tratados, tareas asignadas, aspectos a trabajar en próximas sesiones..." data-v-f7095ca9>${ssrInterpolate(notas.value)}</textarea><div class="mt-3 flex items-center justify-between" data-v-f7095ca9><p class="text-xs text-[#5D4A44]/50" data-v-f7095ca9> Las notas se guardan automáticamente. Solo tú puedes verlas. </p><button class="px-4 py-2 bg-[#D8AFA0] hover:bg-[#C89B8A] text-white rounded-lg transition-colors text-sm font-medium" data-v-f7095ca9> Guardar ahora </button></div></div></div><div class="space-y-6" data-v-f7095ca9><div class="bg-white rounded-xl shadow-sm p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><h3 class="font-[&#39;Lora&#39;] text-lg text-[#5D4A44] mb-4" data-v-f7095ca9>Acciones</h3><div class="space-y-3" data-v-f7095ca9>`);
        if (sesion.value.estado === "pendiente") {
          _push(`<button class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors text-sm font-medium" data-v-f7095ca9> ✓ Confirmar Sesión </button>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (["pendiente", "confirmada"].includes(sesion.value.estado)) {
          _push(`<button class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors text-sm font-medium" data-v-f7095ca9> ✕ Cancelar Sesión </button>`);
        } else {
          _push(`<!---->`);
        }
        _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        _push(`<button class="w-full px-4 py-2 bg-white border border-[#EAD5D3] text-[#5D4A44] hover:bg-[#F9F7F3] rounded-lg transition-colors text-sm font-medium" data-v-f7095ca9> 📄 Generar Recibo </button></div></div><div class="bg-[#F9F7F3] rounded-xl p-6 border border-[#EAD5D3]/40" data-v-f7095ca9><h4 class="font-[&#39;Lora&#39;] text-sm text-[#5D4A44] mb-3" data-v-f7095ca9>Información</h4><div class="space-y-2 text-xs text-[#5D4A44]/70" data-v-f7095ca9><p data-v-f7095ca9>• Creada: ${ssrInterpolate(formatearFecha(sesion.value.created_at))}</p><p data-v-f7095ca9>• ID: ${ssrInterpolate(sesion.value.id)}</p>`);
