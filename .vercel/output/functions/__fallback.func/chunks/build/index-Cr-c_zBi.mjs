...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><div class="mb-8"><h2 class="text-3xl font-bold text-gray-900 mb-2">Métricas del Negocio</h2><p class="text-gray-600">Vista general del rendimiento y estado del sistema</p></div>`);
      if (cargando.value) {
        _push(`<div class="flex items-center justify-center py-24"><div class="text-center"><div class="w-16 h-16 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mx-auto mb-4"></div><p class="text-gray-600 font-medium">Cargando métricas...</p></div></div>`);
      } else {
        ...
        _push(`</div></div><h4 class="text-sm font-semibold text-gray-600 mb-1">Ocupación Real</h4><p class="text-3xl font-bold text-gray-900">${ssrInterpolate(metricas.value.tasaOcupacionReal)}%</p><p class="text-xs text-gray-500 mt-2">Confirmadas + Realizadas</p></div><div class="bg-white rounded-xl p-6 shadow-md border border-gray-200"><div class="flex items-center justify-between mb-4"><div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">`);
        _push(ssrRenderComponent(unref(CalendarIcon), { class: "w-7 h-7 text-blue-600" }, null, _parent));
        _push(`</div></div><h4 class="text-sm font-semibold text-gray-600 mb-1">Ocupación Proyectada</h4><p class="text-3xl font-bold text-gray-900">${ssrInterpolate(metricas.value.tasaOcupacionProyectada)}%</p><p class="text-xs text-gray-500 mt-2">Últimos 30 días</p></div><div class="bg-white rounded-xl p-6 shadow-md border border-gray-200"><div class="flex items-center justify-between mb-4"><div class="w-12 h-12 bg-teal-100 rounded-lg flex items-center justify-center">`);
        _push(ssrRenderComponent(unref(CalendarIcon), { class: "w-7 h-7 text-teal-600" }, null, _parent));
        _push(`</div></div><h4 class="text-sm font-semibold text-gray-600 mb-1">Previsión Próxima Semana</h4><p class="text-3xl font-bold text-gray-900">${ssrInterpolate(metricas.value.ocupacionProximaSemana)}%</p><p class="text-xs text-gray-500 mt-2">Citas programadas</p></div><div class="bg-white rounded-xl p-6 shadow-md border border-gray-200"><div class="flex items-center justify-between mb-4"><div class="w-12 h-12 bg-amber-100 rounded-lg flex items-center justify-center">`);
        _push(ssrRenderComponent(unref(ArrowTrendingDownIcon), { class: "w-7 h-7 text-amber-600" }, null, _parent));
        _push(`</div></div><h4 class="text-sm font-semibold text-gray-600 mb-1">Tasa de Abandono</h4><p class="text-3xl font-bold text-gray-900">${ssrInterpolate(metricas.value.churnRate)}%</p><p class="text-xs text-gray-500 mt-2">Últimos 3 meses</p></div></div></section><section class="mb-8"><h3 class="text-xl font-bold text-gray-900 mb-4">Resumen General</h3><div class="grid grid-cols-1 md:grid-cols-3 gap-6"><div class="bg-white rounded-xl p-6 shadow-md border border-gray-200"><div class="flex items-center gap-4 mb-4"><div class="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center">`);
        ...
        _push(`</tbody></table></div></div></section><div class="text-center text-sm text-gray-500"> Última actualización: ${ssrInterpolate(fechaActualizacion.value)}</div><!--]-->`);
      }
      _push(`</div>`);
    };
  }
...
//# sourceMappingURL=index-Cr-c_zBi.mjs.map
