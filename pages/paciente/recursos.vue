<template>
  <div class="max-w-4xl mx-auto space-y-6 px-4 py-8">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[40vh]">
      ...
      <p class="mt-4 text-[#5D4A44] opacity-70 font-['Lato']">Cargando recursos...</p>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="recursos.length > 0 && recursos[0]?.id?.startsWith('demo')" class="mb-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            <h3 class="font-['Lora'] text-lg font-semibold text-purple-900 mb-1">
      ...
      <div v-if="recursos.length > 0" class="space-y-4">
        ...
        >
          <div class="flex items-start gap-4">
            ...
            <div class="flex-shrink-0 w-12 h-12 rounded-lg bg-[#D8AFA0]/10 flex items-center justify-center">
              <!-- PDF -->
              <svg v-if="recurso.tipo === 'pdf'" class="w-6 h-6 text-[#D8AFA0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
              <!-- Audio -->
              <svg v-else-if="recurso.tipo === 'audio'" class="w-6 h-6 text-[#D8AFA0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" />
              </svg>
              <!-- Video -->
              <svg v-else-if="recurso.tipo === 'video'" class="w-6 h-6 text-[#D8AFA0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
              </svg>
              <!-- Link -->
              <svg v-else class="w-6 h-6 text-[#D8AFA0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-4 mb-2">
                <h3 class="text-lg font-['Lora'] font-medium text-[#5D4A44]">
                  {{ recurso.titulo }}
                </h3>
                <span class="flex-shrink-0 px-2 py-1 rounded-full text-xs font-['Lato'] font-medium" :class="getTipoClass(recurso.tipo)">
                  {{ getTipoTexto(recurso.tipo) }}
                </span>
              </div>

              <p v-if="recurso.descripcion" class="text-sm text-[#5D4A44] opacity-70 font-['Lato'] mb-3">
                {{ recurso.descripcion }}
              </p>

              <!-- Nota personal del terapeuta -->
              <div v-if="recurso.nota_personal" class="bg-[#D8AFA0]/10 border-l-4 border-[#D8AFA0] p-3 mb-3 rounded-r">
                <p class="text-xs font-semibold text-[#5D4A44] mb-1">💬 Nota de tu terapeuta:</p>
                <p class="text-sm text-[#5D4A44] italic">{{ recurso.nota_personal }}</p>
              </div>

              <!-- Categoría y tags -->
              <div v-if="recurso.categoria || recurso.tags?.length" class="mb-3 flex flex-wrap gap-2">
                <span v-if="recurso.categoria" class="px-2 py-1 bg-[#EAD5D3]/30 text-[#5D4A44] rounded text-xs">
                  {{ recurso.categoria }}
                </span>
                <span 
                  v-for="tag in recurso.tags?.slice(0, 3)" 
                  :key="tag"
                  class="px-2 py-1 bg-[#F9F7F3] text-[#5D4A44]/70 rounded text-xs"
                >
                  #{{ tag }}
                </span>
              </div>
              ...
              <!-- Info adicional -->
              <div class="flex items-center gap-4 text-xs text-[#5D4A44] opacity-60 font-['Lato'] mb-4">
                ...
                <span v-if="!recurso.visto" class="px-2 py-0.5 bg-[#D8AFA0] text-white rounded-full text-xs font-medium">
                  Nuevo
                </span>
              </div>
              ...
              <div class="flex gap-2">
                ...
                <a 
                  ...
                  class="inline-flex items-center gap-2 px-4 py-2 bg-[#D8AFA0] hover:bg-[#C89B8A] text-white rounded-lg text-sm font-['Lato'] font-medium transition-colors"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-else class="flex flex-col items-center justify-center text-center py-16">
        ...
        <h3 class="text-xl font-['Lora'] font-medium text-[#5D4A44] mb-2">No hay recursos disponibles</h3>
