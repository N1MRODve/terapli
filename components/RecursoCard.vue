<template>
  <div class="bg-white rounded-xl shadow-sm border border-[#E2E8F0]/30 p-6 hover:shadow-md transition-all duration-300 group">
    <!-- Header con título y tipo -->
    <div class="flex items-start justify-between gap-4 mb-4">
      <div class="flex-1 min-w-0">
        <h3 class="font-serif text-lg text-[#2D3748] mb-1 truncate group-hover:text-[#5550F2] transition-colors">
          {{ recurso.titulo }}
        </h3>
        <p class="text-sm text-[#2D3748]/70 line-clamp-2">
    ...
    <div v-if="mostrarInfo && recurso.creado_por_info" class="flex items-center gap-2 mb-4 text-xs text-[#2D3748]/60">
    ...
    <div v-if="mensaje" class="mb-4 p-3 bg-[#F2F2F2] rounded-lg border border-[#E2E8F0]/30">
      <p class="text-sm text-[#2D3748]/80 italic">
        "{{ mensaje }}"
      </p>
    </div>
    ...
    <div v-if="mostrarBadgeNoVisto && !visto" class="mb-4">
      <span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-[#5550F2]/20 text-[#5550F2] border border-[#5550F2]/30">
    ...
    <!-- Acciones -->
    <div class="flex items-center justify-between gap-3 pt-4 border-t border-[#E2E8F0]/20">
      <!-- Ver recurso -->
      <a 
        :href="recurso.url" 
        target="_blank" 
        rel="noopener noreferrer"
        class="flex items-center gap-2 text-[#5550F2] font-sans text-sm hover:text-[#2D3748] transition-colors"
      ...
      <div class="flex items-center gap-2">
        ...
        <button 
          ...
          class="flex items-center gap-1.5 text-sm text-[#2D3748] bg-[#F2F2F2] border border-[#E2E8F0]/50 px-3 py-1.5 rounded-lg hover:bg-[#E2E8F0]/20 hover:border-[#E2E8F0] transition-all"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const obtenerEstiloTipo = (tipo) => {
  const estilos = {
    ...
    'Guía': 'bg-[#E2E8F0]/50 text-[#2D3748] border border-[#E2E8F0]',
