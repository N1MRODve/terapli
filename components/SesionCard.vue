<template>
  <div 
    :class="[
      ...
      pasada ? 'border-[#EAD5D3]/50 opacity-90' : 'border-[#EAD5D3] hover:border-[#D8AFA0]/50 hover:shadow-md'
    ]"
  >
    <div class="space-y-4">
      <!-- Fecha y estado -->
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
          <h4 class="text-base font-['Lora'] font-medium text-[#5D4A44] mb-1">
            {{ formatearFechaLarga(sesion.fecha) }}
          </h4>
          <div class="flex items-center space-x-3 text-sm text-[#5D4A44]/70 font-['Lato']">
      ...
      <div class="flex items-center space-x-2">
        <span class="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-xs font-['Lato'] bg-[#F9F7F3] text-[#5D4A44]">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Nota del terapeuta -->
      <div v-if="sesion.nota_terapeuta && sesion.estado === 'realizada'" class="bg-[#F9F7F3] rounded-lg p-4 space-y-2 border border-[#EAD5D3]/50">
        <p class="text-xs font-['Lato'] font-semibold text-[#D8AFA0] flex items-center space-x-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          class="text-xs font-['Lato'] font-medium text-[#D8AFA0] hover:text-[#c99d8d] transition-colors"
        >
      ...
      <div>
        <button
          ...
          class="w-full inline-flex items-center justify-center space-x-2 px-4 py-2.5 bg-[#D8AFA0] text-white rounded-lg hover:bg-[#c99d8d] transition-all duration-200 font-['Lato'] font-medium text-sm"
        >
        ...
        <div v-else-if="sesion.ubicacion && sesion.modalidad === 'presencial'" class="flex items-start space-x-2 text-xs font-['Lato'] text-[#5D4A44]/60 bg-[#F9F7F3] px-3 py-2 rounded-lg">
          <svg class="w-4 h-4 text-[#D8AFA0] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
